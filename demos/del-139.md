...
Issue:
- New order alerts on Chickita POS are easy to miss due to a single, short sound. No voice or repeat alert is present.
Solution:
...
Solution:
- When the user is on the /order page, automatically trigger a table refresh to fetch and display the latest data as soon as a new order is received. Also apply this logic for new reservations if applicable.
