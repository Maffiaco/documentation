...
In "Discover Now" page -> "Dine-in" tab -> Select "Order Now" button of Backroom Thao Dien.
Then get error 500 - Internal Server Error.
Expect: hide "Order Now" button of this brand.

Root Cause: because the backroom brand does not have `brandSlug`.
Fix: Add `brandSlug` into the showing condition of the `Order Now` button.
...
Expect: update "Bạn có thể đặt món trước" to "Bạn có thể đặt món/đặt bàn trước" for closing location for VI lang.
...
Open Add order for booking
Click "Save & Send" (not added items yet)
Add items for order
Click "Save"
Then items auto sent to KDS

Expect: Save items and not sent to KDS in this scenario
...
Currently, when user adds products to the Category, displays deactivated products. It's easy to get confused.
